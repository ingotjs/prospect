...
const e2e = defineE2ECoverage({
  ...
  routes: {
    "/": {
      interactions: {
        [testId.counter.buttonIncrement]: [
          { expected: "increments the counter", test: "tests/example.e2e.ts" },
        ],
        [testId.counter.buttonReset]: [
          { expected: "resets the counter to 0", test: "tests/example.e2e.ts" },
        ],
        [testId.form.inputName]: [
          { expected: "accepts text input", test: null },
        ],
        ...
        [testId.links.linkGithub]: [
          { expected: "navigates to GitHub repo", test: null },
        ],
        [testId.links.linkDocs]: [
          { expected: "navigates to docs", test: null },
        ],
      },
    },
  },
